...
class SimpleRenjuWrapper extends StatefulWidget {
  ...
  @override
  State<SimpleRenjuWrapper> createState() => _SimpleRenjuWrapperState();
}

class _SimpleRenjuWrapperState extends State<SimpleRenjuWrapper> {
  String? _currentWarning;

  @override
  Widget build(BuildContext context) {
    final size = widget.boardSize ?? MediaQuery.of(context).size.width * 0.9;
    ...
    return Stack(
      ...
      children: [
        ...
        // 렌주룰 금지 위치 오버레이
        Positioned.fill(child: _buildForbiddenOverlay(size)),
  ...
  Widget _buildForbiddenOverlay(double size) {
    ...
    for (int row = 0; row < boardSize; row++) {
      for (int col = 0; col < boardSize; col++) {
        if (widget.gameState.board[row][col] == null) {
          ...
          if (forbiddenType != ForbiddenType.none) {
            markers.add(
              Positioned(
                left: col * tileSize + (tileSize - 24) / 2,
                top: row * tileSize + (tileSize - 24) / 2,
                child: GestureDetector(
                  onTap: () => _handleForbiddenTap(row, col, forbiddenType),
                  child: Container(
                    width: 24,
                    height: 24,
  ...
  void _handleForbiddenTap(int row, int col, ForbiddenType forbiddenType) {
    // 흑돌만 렌주룰 적용
    if (widget.gameState.currentPlayer != PlayerType.black) {
      widget.onTileTap(row, col);
      return;
    }

    print('🚫 금지 수 감지: ($row, $col) - $forbiddenType');
